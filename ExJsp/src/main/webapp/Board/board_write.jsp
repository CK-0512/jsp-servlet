...
<script type="text/javascript">
	function send(){
		let board = document.getElementsByName("board")[0];
		if(!board.name.value.trim().length){
			alert("이름을 입력하세요");
			board.name.focus();
			return;
		}
		if(!board.subject.value.trim().length){
			alert("제목을 입력하세요");
			board.subject.focus();
			return;
		}
		if(!board.contents.value.trim().length){
			alert("내용을 입력하세요");
			board.contents.focus();
			return;
		}
		if(!board.pass.value.trim().length){
			alert("비밀번호를 입력하세요");
			board.pass.focus();
			return;
		}
 ...
 <tr>
   ...
   <td width="80%" valign="top">&nbsp;<br>
     ...
     <form method="post" action="board_write_pro.jsp" name="board">
