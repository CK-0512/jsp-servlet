...
//		pstmt.setString(1, "%" + search + "%");
	...
	public List<PdsDTO> listPds(int pagestart, int endpage)  {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	//자료실 검색 + 페이지별 데이터 검색
	public List<PdsDTO> listPds(String s_query, int pagestart, int endpage)  {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select X.* from (select rownum as rnum, A.* from (" + 
...
//		pstmt.setString(1, "%" + search + "%");
	...
	public int insertPds(PdsDTO pVo) {
	...
	public PdsDTO selectPdsNum(int idx) {
	...
	public int updatePds(PdsDTO pVo) {
